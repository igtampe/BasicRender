...
namespace Igtampe.BasicShapes {
    ...
    public class Curve : Line {
        ...
        /// <returns></returns>
        public override bool Equals(object obj) => obj is Curve C2
            ? A1 == C2.A1 && A2 == C2.A2 && Center == C2.Center
            : base.Equals(obj);
        ...
        /// <returns></returns>
        public override string ToString() => "Curve from " + A1 + " to " + A2 + " with center " + Center + " and radius " + R;
        ...
        internal static double DegToRadians(double Degree) => Degree * (Math.PI / 180);
        ...
        internal static PointF CalculatePoint(double Angle, PointF Center, double Radius)
            => new PointF(Convert.ToSingle(Center.X + (Math.Cos(DegToRadians(Angle)) * Radius)), Convert.ToSingle(Center.Y + (Math.Sin(DegToRadians(Angle)) * Radius)));
        ...
        /// <param name="DY"></param>
        /// <returns></returns>
        public static Curve TranslateCurve(Curve L, float DX, float DY) => new Curve(new PointF(L.Center.X + DX, L.Center.Y + DY), L.R, L.A1, L.A2);

        /// <summary>Scales a line's length up or down, anchored on a center point P3</summary>
        /// <param name="L"></param>
        /// <param name="scale"></param>
        /// <returns></returns>
        public static Curve ScaleCurve(Curve L, double scale) => new Curve(L.Center, L.R * scale, L.A1, L.A2);
    }
}
